...
if (equipJson) {
    convert('../put.xls', '../../putsrc/assets/configs/EquipIdsByLvRank.ts', 'equip', 'equipIdsByLvRank', null, function (data) {
        ...
        for (const equipId in equipJson) {
            ...
            const equipData = equipJson[equipId];

            if (equipIds[equipData.lv] === undefined) {
                equipIds[equipData.lv] = [];
            }

            if (equipIds[equipData.lv][equipData.rank - 1] === undefined) {
                equipIds[equipData.lv][equipData.rank - 1] = [];
            }
            equipIds[equipData.lv][equipData.rank - 1].push(equipId);
        }

        for (let index = 0; index < equipIds.length; index++) {
            const element = equipIds[index];
            if (!element) equipIds[index] = [];
        }
